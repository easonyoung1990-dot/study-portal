...
7. **生词默写/听写 = 用"默写助手"那套机制（⭐ 凡是默写类软件都按它做）**：参考实现
   ...
   - **错词重做 + 错词本（⭐）**：可基于历史错词单独生成"错词重做"软件，沿用同一引擎（见 `apps/kenton_dictation_redo.html`、`apps/eddey_english_redo.html`）。
     机制＝**错一次就进错词本**；错词**连续答对 N 次（默认 2 次、第一次答对不算）才算掌握、移出错词本**；错词本可自动吸收主默写软件归档里答错的词。
